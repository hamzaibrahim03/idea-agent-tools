import { useEffect, useState } from 'react';
const DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
function emptyClass() {
    return { name: '', day: 'Monday', start: '09:00', end: '10:00' };
}
export default function ClassScheduleBuilder() {
    const [classes, setClasses] = useState([
        { name: 'Calculus', day: 'Monday', start: '09:00', end: '10:00' },
        { name: 'Chemistry', day: 'Wednesday', start: '11:00', end: '12:30' }
    ]);
    const [conflicts, setConflicts] = useState([]);
    const [grid, setGrid] = useState([]);
    const [error, setError] = useState('');
    function updateClass(i, field, value) {
        setClasses((prev) => prev.map((c, idx) => (idx === i ? { ...c, [field]: value } : c)));
    }
    useEffect(() => {
        let cancelled = false;
        setError('');
        fetch('/api/tools/class-schedule-builder', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ input: { classes } })
        })
            .then((r) => r.json())
            .then((d) => {
                if (cancelled) return;
                if (d.error) setError(d.error);
                else {
                    setConflicts(d.conflicts || []);
                    setGrid(d.grid || []);
                }
            })
            .catch((e) => { if (!cancelled) setError(e.message || 'Failed to compute'); });
        return () => { cancelled = true; };
    }, [classes]);
    const conflictSet = new Set(conflicts);
    return (
        <div className="tool-page">
            <h1>Class Schedule Builder</h1>
            <p className="tool-description">
                Add your classes with day and start/end time to see a weekly grid view, with any
                overlapping time conflicts on the same day flagged automatically.
            </p>
            {error && <div className="agent-error">{error}</div>}
            <div className="tool-controls">
                <button type="button" onClick={() => setClasses((prev) => [...prev, emptyClass()])}>
                    Add class
                </button>
            </div>
            <ul className="uuid-list">
                {classes.map((c, i) => (
                    <li key={i} style={{ background: conflictSet.has(i) ? 'rgba(220, 38, 38, 0.1)' : undefined }}>
                        <input type="text" placeholder="Class name" value={c.name} onChange={(e) => updateClass(i, 'name', e.target.value)} style={{ flex: 1 }} />
                        <select value={c.day} onChange={(e) => updateClass(i, 'day', e.target.value)}>
                            {DAYS.map((d) => (
                                <option key={d} value={d}>
                                    {d}
                                </option>
                            ))}
                        </select>
                        <input type="time" value={c.start} onChange={(e) => updateClass(i, 'start', e.target.value)} />
                        <span>to</span>
                        <input type="time" value={c.end} onChange={(e) => updateClass(i, 'end', e.target.value)} />
                        {conflictSet.has(i) && <span className="tool-error-inline">Conflict</span>}
                        <button type="button" className="uuid-copy-btn" onClick={() => setClasses((prev) => prev.filter((_, idx) => idx !== i))} disabled={classes.length <= 1} >
                            Remove
                        </button>
                    </li>
                ))}
            </ul>
            {conflictSet.size > 0 && (
                <div className="tool-error">
                    <strong>Schedule conflict:</strong> {conflictSet.size} class(es) have overlapping times on the same day - highlighted above.
                </div>
            )}
            <h2 style={{ fontSize: 16, margin: '16px 0 8px' }}>Weekly grid</h2>
            <div className="regex-groups-wrap">
                <table className="regex-groups-table">
                    <thead>
                        <tr>
                            <th>Day</th>
                            <th>Classes</th>
                        </tr>
                    </thead>
                    <tbody>
                        {grid.map((row) => (
                            <tr key={row.day}>
                                <td>{row.day}</td>
                                <td>
                                    {row.classes.length === 0
                                        ? '—'
                                        : row.classes
                                            .map((c) => `${c.name || '(untitled)'} (${c.start}-${c.end})${conflictSet.has(c.idx) ? ' [conflict]' : ''}`)
                                            .join(', ')}
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
            </div>
        </div>
    );
}
