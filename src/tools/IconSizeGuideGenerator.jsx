...
export default function IconSizeGuideGenerator() {
    ...
    useEffect(() => {
        let cancelled = false;
        fetch('/api/tools/icon-size-guide', {
            method: 'POST',
                ...
                <div className="timestamp-result">
                    {items.map((s) => (
                        <span key={s.px} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            <span style={{ width: s.px, height: s.px, minWidth: s.px, border: '2px solid var(--accent-border)', borderRadius: 4, background: 'var(--accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} />
