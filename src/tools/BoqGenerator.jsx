...
export default function BoqGenerator() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setError('');
            fetch('/api/tools/boq-generator', {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ input: { projectName, items } })
            })
                .then((r) => r.json())
                .then((data) => {
                    if (cancelled) return;
    ...
    return (
        <div className="tool-page">
            ...
            <p className="tool-description">
                ...
                substitute for a licensed quantity surveyor's official BOQ.
            </p>
            <div className="tool-panel">
                <label htmlFor="project-name">Project name (optional)</label>
                <input id="project-name" type="text" value={projectName} onChange={(e) => setProjectName(e.target.value)} placeholder="e.g. Riverside Residence - Phase 1" />
            </div>
                    ...
                    <tbody>
                        {items.map((it, i) => (
                            <tr key={i}>
                                <td>{i + 1}</td>
                                <td>
                                    <input type="text" value={it.description} onChange={(e) => updateItem(i, 'description', e.target.value)} placeholder="e.g. Brickwork in cement mortar" style={{ width: '100%' }} />
                                </td>
                                <td>
                                    <input type="text" value={it.unit} onChange={(e) => updateItem(i, 'unit', e.target.value)} placeholder="m2" style={{ width: '70px' }} />
                                </td>
                                <td>
                                    <input type="number" min={0} value={it.quantity} onChange={(e) => updateItem(i, 'quantity', e.target.value)} style={{ width: '80px' }} />
                                </td>
                                <td>
                                    <input type="number" min={0} value={it.rate} onChange={(e) => updateItem(i, 'rate', e.target.value)} style={{ width: '90px' }} />
                                </td>
                                <td>
                                    <code>{rows[i]?.validRow ? rows[i].amount.toFixed(2) : '—'}</code>
                                </td>
                                <td>
