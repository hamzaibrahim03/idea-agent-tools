...
export default function HashGenerator() {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        setFetchError('');
        fetch('/api/tools/hash-generator', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ input: { input, algorithm } })
        })
            .then((r) => r.json())
            .then((data) => {
                if (cancelled) return;
                if (data.error) setFetchError(data.error);
                else setHash(data.hash || '');
            })
    ...
    return (
        <div className="tool-page">
            ...
            <div className="tool-panel">
                ...
                <textarea id="hash-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type or paste text to hash" spellCheck={false} />
            </div>
