    ...
    return (
        <div className="tool-page">
            ...
            <div className="tool-controls">
                <label>
                    Number of dice:
                    <input type="number" min={1} max={10} value={diceCount} onChange={(e) => setDiceCount(e.target.value)} style={{ width: '70px' }} />
                </label>
                <label>
                    Die sides:
                    <select value={useCustomSides ? 'custom' : diceSides} onChange={(e) => { if (e.target.value === 'custom') { setUseCustomSides(true); } else { setUseCustomSides(false); setDiceSides(Number(e.target.value)); } }} >
                        {DIE_SIDES.map((s) => (
                ...
                {useCustomSides && (
                    <label>
                        Sides:
                        <input type="number" min={2} value={customSides} onChange={(e) => setCustomSides(e.target.value)} style={{ width: '70px' }} />
                    </label>
                )}
            ...
            <div className="tool-controls">
                <label>
                    Number of coins:
                    <input type="number" min={1} max={10} value={coinCount} onChange={(e) => setCoinCount(e.target.value)} style={{ width: '70px' }} />
                </label>
