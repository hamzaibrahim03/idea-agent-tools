    ...
    return (
        <div className="tool-panel">
            ...
            {mode === 'every' && (
                <label>
                    Every
                    <input type="number" min={1} max={max} value={everyStep} onChange={(e) => setEveryStep(Math.max(1, Number(e.target.value) || 1))} style={{ width: 70, marginLeft: 6 }} />
                    {' '}unit(s)
                </label>
            )}
            {mode === 'fixed' && (
...
export default function CronExpressionBuilder() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setFetchError('');
            const toField = (f) => ({ mode: f.mode, everyStep: f.everyStep, fixedValue: f.fixedValue, selected: [...f.selectedSet] });
            fetch('/api/tools/cron-expression-builder', {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({
                    input: {
                        minute: toField(minute),
                        hour: toField(hour),
                        dayOfMonth: toField(dayOfMonth),
                        month: toField(month),
                        dayOfWeek: toField(dayOfWeek)
                    }
                })
            })
                .then((r) => r.json())
        ...
        minute.mode, minute.everyStep, minute.fixedValue, minute.selectedSet,
        hour.mode, hour.everyStep, hour.fixedValue, hour.selectedSet,
        dayOfMonth.mode, dayOfMonth.everyStep, dayOfMonth.fixedValue, dayOfMonth.selectedSet,
        month.mode, month.everyStep, month.fixedValue, month.selectedSet,
        dayOfWeek.mode, dayOfWeek.everyStep, dayOfWeek.fixedValue, dayOfWeek.selectedSet
    ]);
    ...
    return (
        <div className="tool-page">
            ...
            <div className="tool-panel">
                ...
                <input id="cron-builder-output" type="text" value={expression} readOnly style={{ fontFamily: 'var(--mono)' }} />
            </div>
        </div>
    );
}
