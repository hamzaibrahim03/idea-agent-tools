    ...
    return (
        <div className="tool-page">
            ...
            <div className="tool-panel">
                <label>Preview</label>
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, 1fr)`, gridTemplateRows: `repeat(${rows}, 1fr)`, gap: `${gap}px`, padding: 16, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--code-bg)', }} >
                    {Array.from({ length: cellCount }, (_, i) => (
                        <div key={i} style={{ minHeight: 48, borderRadius: 6, background: 'var(--accent-border)', color: 'var(--text-h)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, }} >
                            {i + 1}
                        </div>
                    ))}
                </div>
