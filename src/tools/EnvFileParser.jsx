...
export default function EnvFileParser() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setFetchError('');
            fetch('/api/tools/env-file-parser', {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ input: { input, rows } })
            })
                .then((r) => r.json())
                .then((data) => {
                    if (cancelled) return;
                    if (data.error) {
                        ...
                        setGeneratedEnv(data.generatedEnv || '');
                    }
                })
    ...
    return (
        <div className="tool-page">
            ...
            <div className="tool-panel">
                ...
                <textarea id="env-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="KEY=value" spellCheck={false} />
            </div>
            ...
            <div className="tool-panel">
                ...
                {rows.map((r, i) => (
                    <div className="tool-controls" key={i}>
                        <input type="text" value={r.key} onChange={(e) => updateRow(i, 'key', e.target.value)} placeholder="KEY" style={{ width: '180px' }} />
                        <input type="text" value={r.value} onChange={(e) => updateRow(i, 'value', e.target.value)} placeholder="value" style={{ width: '220px' }} />
